...
| Notificação | Armazenar notificações exibidas aos usuários. |
...
| `mesa_id` | Mesa associada à comanda. |
...
Armazenar notificações destinadas aos funcionários.

As notificações serão utilizadas principalmente para informar que pedidos estão prontos.
...
| `usuario_id` | Usuário que receberá a notificação. |
| `pedido_id` | Pedido relacionado, quando aplicável. |
| `titulo` | Título da notificação. |
| `mensagem` | Conteúdo da notificação. |
| `lida` | Indica se a notificação foi visualizada. |
| `created_at` | Data e horário da criação. |

## Relacionamentos

Um usuário pode possuir várias notificações.

Uma notificação pode estar relacionada a um pedido.
...
# 16. Relacionamentos e Cardinalidades
...
# 17. Isolamento Multi-Restaurante
...
- Um pedido deve possuir pelo menos um item para ser enviado à cozinha.
